...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var soccerTable = $('#soccerTable').DataTable({
        responsive: true,
        searching:true,
        pageLength: -1,
        lengthMenu: [[-1, 10, 25, 50], ["All", 10, 25, 50]],
        ...
        autoWidth: false,
        language: {
                "emptyTable": `No data found in Soccer. Check back later.`,
            },
        columnDefs: [
    ...
    $('#floatingSelectTeam').on('change', function () {
            var selectedStat = $(this).val();
            if (selectedStat === "All") {
                soccerTable.search('').columns().search('').draw();
            } else {
                soccerTable.column(2).search(selectedStat, true, false).draw();
            }
        });

    $('#floatingPlayerNames').on('change', function () {
        var selectedStat = $(this).val();
        if (selectedStat === "All") {
            soccerTable.search('').columns().search('').draw();
        } else {
            soccerTable.column(3).search(selectedStat, true, false).draw();
        }
    });

    $('#floatingStatType').on('change', function () {
        var selectedStat = $(this).val();
        if (selectedStat === "All") {
            soccerTable.search('').columns().search('').draw();
        } else {
    ...
    $('#flexSwitchCheckDefault').on('change', function() {
        if ($(this).prop('checked')) {
            soccerTable.rows().every(function() {
                var data = this.data(); // Get row data
                var value = parseFloat($(data[4]).text().replace("x", "").trim());
    ...
    $('#customSearchSoccer').on('keyup', function() {
        if (soccerTable) {
            // Use DataTable search functionality
            soccerTable.search(this.value).draw();
        }
    });
